...
class Escalonador:
    ...
    def processa_blocked(self, surto_cpu):
        ...
        if len(self.bloqueados) > 0:
            # processa syscalls
            for blocked in self.bloqueados[:]:  # cópia da lista para evitar problemas de modificação durante iteração
    ...
    def executar_simulacao(self):
        ...
        while self.processos or self.bloqueados:
            ...
            if self.processos:
                ...
                for p in [*self.processos, *self.bloqueados, *self.terminados]:
                ...
                if proces.tempo_total_cpu <= 0:
                    ...
                    try:
                        ...
                        self.bloqueados.remove(proces)
                    self.terminados.append(proces)
